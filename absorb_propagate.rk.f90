...
!
  module absorb_propagate 
    ...
    integer(ik)                  :: ntstep           = 5   ! Number of time steps to store (temporary)
...
!      write(out,"('nu=',4(i2),' mu=',4(i2),' c=',f12.8,' cf=',f12.8,' val=',f12.8,' t=',f6.4)")nu(6:9),mu(6:9),c,abs(cf),abs(val),t
    ...
    !
    subroutine accumulate_at(mu,cf,lvl,F,t)
      integer(sik), intent(in) :: mu(:)  ! State of interest
      complex(rk),intent(in)   :: cf     ! Expansion coefficient
      integer(ik),intent(in)   :: lvl    ! where to accumulate contrib.
      real(rk),intent(in)      :: F(3),t
      !
      ...
      if(abs(at) > 0_rk) call update_cf(nu,1_ik,lvl,at,ovrwrite=.false.)
    ...
    !
    !
    subroutine accumulate_bt(mu,cf,typ,lvl,F,t)
      integer(sik), intent(in) :: mu(:)  ! State of interest
      complex(rk),intent(in)   :: cf     ! Expansion coefficient
      integer(ik), intent(in)  :: typ,lvl ! term type,where to accumulate contrib
      ...
      if(abs(bt) > 0._rk)call update_cf(nu,2_ik,lvl,bt,ovrwrite=.false.)
      !
      quantum_c1: do c1=gs%nvarsDummy+1,gs%nvars
        !
...
!        endif
    ...
    ! update active slot
    !
    subroutine update_active_arg(coef,k)
      implicit none
      ...
      call walker_gr_initialize(cache,walker)
       scan_tree: do while(walker_gr_next(walker,n))
        ...
        scan_states: do s = 1,2
         cgr%values(1,s) = cgr%values(0,s)
         if(k > 1)cgr%values(1,s) = cgr%values(1,s) - im * coef * cgr%values(k+1,s)
        end do scan_states
       end do scan_tree
      call walker_gr_destroy(walker)

      return
    end subroutine update_active_arg

    !
    ! add in contribution to all connected del_a/del_t terms
    !
    subroutine update_tderiv(t)
      implicit none
      real(rk),intent(in)      :: t
      ...
      complex(rk)              :: cf
      integer(ik)              :: k
      real(rk)                 :: F(3),tval(4),cval(4)

      tval = (/    t, t+0.5_rk*tstep, t+0.5_rk*tstep, t+tstep /)
      cval = (/0._rk,   0.5_rk*tstep,   0.5_rk*tstep,   tstep /) 

      ! compute k1,k2,k3, and k4
      do k = 1,4

       ! evaluate field at appropriate time
       F = evaluateExternalField( FPulse,tval(k) ) * pulse_dir 

       ! 
       ! load coefs from appropriate point in interval
       !
       call update_active_arg(cval(k),k)
       ...
       scan_tree: do while(walker_gr_next(walker,n))
          ...
          cf = retrieve_cf(n,1_ik,1_ik)
          if(abs(cf) > constant_cutoff ) then
           call accumulate_bt(n,cf,1_ik,k+1,F,tval(k))
          endif

          ! determine contribution from b coefficients
          cf = retrieve_cf(n,2_ik,1_ik)
          if(abs(cf) > constant_cutoff ) then
           call accumulate_at(n,cf,k+1,F,t)
           call accumulate_bt(n,cf,0_ik,k+1,F,tval(k))
          endif
    ...
    subroutine update_position
      ...
      scan_tree: do while(walker_gr_next(walker,n))
        cgr => cache_gr_locate(cache,n)
        scan_states: do s = 1,2
         step = cgr%values(2,s) + 2._rk * cgr%values(3,s) + 2._rk * cgr%values(4,s) + cgr%values(5,s) 
         cgr%values(0,s) = cgr%values(0,s) - im * tstep * step / 6._rk
         n_coef(s) = n_coef(s) + 1
        end do scan_states
      end do scan_tree
      call walker_gr_destroy(walker)
...
!        if(abs(cf)**2 > 1)write(out,"('problem: ',12(i2),' |cf|=',f8.6)")n(7:18),abs(cf)
    ...
    subroutine evolve_wavepacket
      ...
      time_step: do
        ...
        F = evaluateExternalField( FPulse,t ) * pulse_dir

        nrm = wp_norm()
        nrmt = sqrt(nrm(1)**2 + nrm(2)**2)
        write(out,"('t= ',f8.2,' Ft= ',3(f8.5,1x),' |0|=',f8.6,' |1|=',f8.6,' |total|=',f8.6)")t,F,nrm(1),nrm(2),nrmt
